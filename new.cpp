...
void draw ()
{
  ...
  cout<<"camera"<< camera.state<<endl;
  if(camera.state==12213132){
    camera.x=player.x-0.1;
    camera.y=player.y+0.1;
    camera.z=player.z-0.1;
    target=glm::vec3(player.x+4,0,player.z+4);
  }
 ...
 {
    printf("camera.x %d,camera.y %d,camera.z %d\n",camera.x,camera.y,camera.z);

 }  
  Matrices.view = glm::lookAt(glm::vec3(camera.x,camera.y,camera.z), target, glm::vec3(0,1,0)); // Fixed camera for 2D (ortho) in XY plane
